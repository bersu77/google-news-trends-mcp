...
def _get_jwt_signing_key(token: str) -> str:
    ...
    if load_var._MCP_JWT_PUBLIC_KEY:
        # For HS256 (symmetric key), return the secret directly
        # For asymmetric algorithms (RS256, ES256), normalize as PEM
        if load_var._MCP_JWT_ALGORITHMS and 'HS256' in load_var._MCP_JWT_ALGORITHMS:
            return load_var._MCP_JWT_PUBLIC_KEY
        else:
